...
clientes_compras<-frecuencia_clientes %>%
  ...
  head(10)
mejores_clientes<-ggplot(clientes_compras, aes(x = id_cliente_enc, y = compras), fill = colors) +
  geom_col() +
...
datos$dow <- lubridate::wday(datos$dia, label = TRUE, abbr = FALSE, week_start = 1)
...
p15 <- ggplot(heatmap_data, aes(x = mes, y = descripcion, fill = n)) +
  ...
  labs(title = "Top 10 Productos más Vendidos por Mes", 
