...
recomendacion_lista <- list(recomendacion = recomendacion_promocional)
saveRDS(recomendacion_lista, file = "Resultados\\resultados_Objetivo1.rds")
...
ggplot(scores_all, aes(x = reorder(Cliente, -Puntuacion), y = Puntuacion)) +
...
ggplot(recomendacion_promocional, aes(x = reorder(Cliente, Puntuacion), y = Puntuacion)) +
